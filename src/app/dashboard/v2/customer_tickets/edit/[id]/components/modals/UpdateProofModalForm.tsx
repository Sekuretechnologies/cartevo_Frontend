...
export const formSchema: any = z.object({
	proof: z.instanceof(FileList).refine(
		(file) => {
...
export default function UpdateProofModalForm({
	isOpen,
	setIsOpen,
	chnPayment,
}: ModalProps) {
	...
	const mutation = useMutation({
		mutationFn: (data) =>
			handleUpdate({
				...
				chnPaymentId: chnPaymentData?.chnpayment.id,
				body: data,
			}),
		onError: (err: any) => {
			console.error("onError : ", err.message);
			toast.error(
				`Erreur mise a jour de la preuve de paiement : ` + err.message
			);
		},
		onSuccess: (data) => {
			console.log("onSuccess : ", data);
			toast.success(
				`La preuve de paiement a été mise à jour avec succès.`
			);
