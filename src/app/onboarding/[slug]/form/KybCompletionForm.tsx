...
import { z } from "zod";
// import { completeKybSchema } from "@/validation/FormValidation";
...
export const completeKybSchema = z.object({
...
const handleCompleteKyb = async (
	token: string,
	data: z.infer<typeof completeKybSchema>
) => {
	//-------------------------------------------------------
	...
	//-------------------------------------------------------
	const response: any = await CompanyService.completeKyb({
		token,
		body: formData,
	});
	if (!response.ok) {
...
export default function KybCompletionForm() {
	...
	const form = useForm<z.infer<typeof completeKybSchema>>({
		resolver: zodResolver(completeKybSchema),
		defaultValues: {
	...
	const mutation = useMutation({
		mutationFn: (data: z.infer<typeof completeKybSchema>) =>
			handleCompleteKyb(currentToken, data),
		onError: (err: any) => {
